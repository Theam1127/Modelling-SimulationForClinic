...
class State {
    static int doctorQueue, doctorStatus, doctor2Status, medicineQueue, nurseStatus, clockTime, doctorExist;
}
...
public class LeongChunYong {
    ...
    public static void start(FutureEventList list, Queue doctorQueue, Queue medicineQueue){
        ...
        if(State.doctorExist == 2){
            System.out.println("Doctor1 is currently not in the clinic");
            System.out.println("Doctor2 is in the clinic");
        }
        else if(State.doctorExist==1){
            System.out.println("Doctor1 is in the clinic");
            System.out.println("Doctor2 is in the clinic");
        }
        
        System.out.println("System states");
        System.out.println("DQ(t) " + State.doctorQueue + " MQ(t) " + State.medicineQueue + " D(t) " + State.doctorStatus + " D2(t) " + State.doctor2Status + " N(t) " + State.nurseStatus);
        ...
        if(list.findSameClockTime()==true){
            ...
            if(sameClockTimeEvent.type.equals("DepartureDoctor2")){
                totalNumberOfPatientNurse++;
                departureDoctor2(list ,sameClockTimeEvent,doctorQueue,medicineQueue);
            }
        ...
        if(nextClockTimeEvent.type.equals("DepartureDoctor2")){
                totalNumberOfPatientNurse++;
                departureDoctor2(list,nextClockTimeEvent,doctorQueue,medicineQueue);
            }
    ...
    private static void arrivalDoctor(FutureEventList list, Event nextClockTimeEvent, Queue doctorQueue, Queue medicineQueue) {
        ...
        if(State.doctorStatus == 0 && State.doctorExist==1){
            ...
            if(State.doctor2Status == 0){
            State.doctor2Status = 1;
            doctorServiceTime = RandomValuesSet.getRandom(RandomValuesSet.serviceTimeDoctor);
            doctorService.add(doctorServiceTime);
            departureTime = State.clockTime + doctorServiceTime;
            totalSpentDoctor += departureTime - State.clockTime;
            list.add(new Event("DepartureDoctor2", departureTime, nextClockTimeEvent.patient));
        }
        else{
            State.doctorQueue++;
    ...
    private static void departureDoctor(FutureEventList list, Event nextClockTimeEvent, Queue doctorQueue, Queue medicineQueue){
        ...
        if(State.doctorExist==1 && State.doctorStatus == 0){
            ...
            departureTime = State.clockTime + doctorServiceTime;
            totalSpentDoctor += departureTime - arrivalTime;
            list.add(new Event("DepartureDoctor2", departureTime, doctorQueue.remove()));
            }
        }
    ...
    private static void departureDoctor2(FutureEventList list, Event nextClockTimeEvent, Queue doctorQueue, Queue medicineQueue){
        int nurseServiceTime, departureTimeNurse;
        if(State.nurseStatus == 0){
            State.nurseStatus = 1;
            nurseServiceTime = RandomValuesSet.getRandom(RandomValuesSet.serviceTimeNurse);
